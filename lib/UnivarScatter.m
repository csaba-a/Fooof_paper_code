...
%                           *'lines': std, 95% SEM, median are represented as
%                           lines
%                           *'boxes':std, 95% SEM, median are represented as
%                           boxes. Default value
...
%                       with std and median of each subset of points.
...
%                       the median,SEM,std will be Width/WhiskersWidthRatio
...
%                       +'medianColor' color(s) of the median line; def. black
...
%% added by PT to stop it crashing when all values are equal
for k=1:size(data,2)
   tmp=data(:,k);
   tmp(isnan(tmp))=[];
   if sum(abs(diff(tmp)))==0
%       data(1:numel(tmp),k)=tmp+(randn(size(tmp))*(.000001+tmp(1))*.00001); 
   end
...
Ind=strmatch('medianColor',stringVars);
medianColorIsArray=false;
if ~isempty(Ind)
    %Check that medianColor takes valid arguments
    if isnumeric(valueVars{Ind}) && all(size(valueVars{Ind}) >= [size(data,2),3])
        medianColor=valueVars{Ind};
        medianColorIsArray=true;
    elseif (isnumeric(valueVars{Ind}) && all(size(valueVars{Ind}) == [1,3])) || ischar(valueVars{Ind})
        medianColor=valueVars{Ind};
    else
        error(sprintf('wrong medianColor input'))
    end
else
    medianColor='k';
end
...
%If we want boxes for the median and std
if strcmp(Whiskers,'box')
    ...
    %rectangle('Position',[i-Width/WhiskersWidthRatio,ymedian-yStd,2*Width/WhiskersWidthRatio,2*yStd ],'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1);
    %plot the median+-SEM box
    
    %rectangle('Position',[i-Width/WhiskersWidthRatio,ymedian-yCI,2*Width/WhiskersWidthRatio,2*yCI ],'FaceColor',SEMColor(SEMIndex,:),'EdgeColor', SEMColor(SEMIndex,:),'LineWidth',0.1);
    
    %Plot median and 25th and 75th percentile
    P_25 = prctile(yValues,25);
    P_75 = prctile(yValues,75);
    rectangle('Position',[i-Width/WhiskersWidthRatio,P_25,2*Width/WhiskersWidthRatio,P_75 ],'FaceColor',SEMColor(SEMIndex,:),'EdgeColor', SEMColor(SEMIndex,:),'LineWidth',0.1);
    
    %plot the median line 
    plot([i-Width/WhiskersWidthRatio i+Width/WhiskersWidthRatio],[yMedian yMedian],'Color',medianColor(medianIndex,:),'LineWidth',2)
...
range_val=norminv([0.05 0.95],median(yValues),std(yValues));
...
% arbitrarily give cuts the value of cuts=median(yValues)/2, this value does not
% really matter because there will only be one group anyway
if isnan(cuts)
    cuts=median(yValues)/2;
end
% The "seed" group contains the values which are in the range of the
% median+-1/2cuts. cutUp is the higher border of the interval. So we
...
while keep_going
    ...
    if all(sum(subsetind)~=[1 0]) %If there's only one point, we represent it in the middle, and xValues remains equal to i
        ...
        % xb is a symmetrical vector with median=0, and the values in it have
...
scatter(xValues,yValues,PointSize,PointStyle,'MarkerEdgeColor', MarkerEdgeColor(EdgeIndex,:),'MarkerFaceColor',MarkerFaceColor(FaceIndex,:),'LineWidth',LineWidth,'MarkerFaceAlpha',.6)
%modified by PT fir niceness
% scatter(xValues,yValues,PointSize,PointStyle,'MarkerEdgeColor', 'none','MarkerFaceColor',MarkerFaceColor(FaceIndex,:),'LineWidth',LineWidth)

%If we want lines to represent the SEM and the std
if strcmp(Whiskers,'lines')
    %plot the median line
    ymedian=median(yValues);
    plot([i-Width/WhiskersWidthRatio i+Width/WhiskersWidthRatio],[ymedian ymedian],'Color',medianColor(medianIndex),'LineWidth',1.5)
    
    %plot the sd 
    yStd=std(yValues);
    plot([i-Width/WhiskersWidthRatio*0.5 i+Width/WhiskersWidthRatio*0.5],[ymedian+yStd ymedian+yStd],'Color',StdColor(StdIndex,:),'LineWidth',1.5)
    plot([i-Width/WhiskersWidthRatio*0.5 i+Width/WhiskersWidthRatio*0.5],[ymedian-yStd ymedian-yStd],'Color',StdColor(StdIndex,:),'LineWidth',1.5)
    plot([i i],[ymedian-yStd ymedian+yStd],'Color',StdColor(StdIndex,:))
    %plot the conf. interval of the median
    ySem=yStd/sqrt(size(yValues,1));
    yCI=ySem*1.96;
    plot([i-Width/WhiskersWidthRatio*0.7 i+Width/WhiskersWidthRatio*0.7],[ymedian+yCI ymedian+yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',1.5)
    plot([i-Width/WhiskersWidthRatio*0.7 i+Width/WhiskersWidthRatio*0.7],[ymedian-yCI ymedian-yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',1.5)
    plot([i i],[ymedian-yCI ymedian+yCI],'Color',SEMColor(SEMIndex,:))
